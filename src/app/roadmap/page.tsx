...
function CycleOverviewCard({ track }: { track: Track }) {
  ...
  const isTrackA = track.id === "track-a"

  return (
    <div className="rounded-xl border border-border/50 bg-card p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {isTrackA ? <Wrench className="h-4 w-4 text-primary/70" /> : <Cpu className="h-4 w-4 text-primary/70" />}
          <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground font-mono">
            {isTrackA ? "Track A" : "Track B"}
          </span>
        </div>
...
export default function RoadmapPage() {
  const [activeTrack, setActiveTrack] = useState<"track-a" | "track-b">("track-a")
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-1.5 p-1 rounded-xl bg-muted/40 border border-border/40 w-fit">
              {tracks.map((t) => (
                <button
                  key={t.id}
                  onClick={() => setActiveTrack(t.id as "track-a" | "track-b")}
                ...
                >
                  {t.id === "track-a" ? <Wrench className="h-4 w-4" /> : <Cpu className="h-4 w-4" />}
                  <span className="hidden sm:inline">{t.name}</span>
                  <span className="sm:hidden">{t.id === "track-a" ? "Physical" : "Simulation"}</span>
                </button>
              ))}
            </div>
